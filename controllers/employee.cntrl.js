...
const employeecntrl = {
    ...
    employeelogin: async function (req, res) {
        try {
            ...
            const employeedeatils=await employeesvc.geEmployeeName(employee._id);
            if (login) {
                //console.log(token);
                
                res.status(200).json({ status: 1, data: { username: req.body.username,empaddress:employeedeatils.currentLocation,mobile: employeedeatils.mobile, token: token } });
            }
            else {
    ...
    driverlogin: async function (req, res) {
        try {
            let driver = await employeesvc.getdriverlogin(req.body.driverusername);
            let login = bcrypt.compareSync(req.body.password, driver.password);
            console.log(JSON.stringify("driver :" + driver +"login :"+login))
            let token = jwt.sign({
                username: req.body.driverusername, id: driver._id
            }, config.secret, { expiresIn: Math.floor(Date.now() / 1000) + (60 * 60) });
            if (login) {
                //console.log(token);
                res.status(200).json({ status: 1, data: { username: req.body.driverusername, token: token } });
            }
            else {
                res.status(200).send({ status: 0, data: { message: 'Invalid username/password' } });
            }
        ...
        try {
            ...
            for (let i = 0; i < alljobs.length; i++) {
                ...
                const companydetails={
                    ...
                    "location":companyname.industryType
                
                }
                temparray.push({ companydetails, "jobDetails": alljobs[i]});
            }
