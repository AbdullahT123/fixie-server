const driversvc = require('../services/driver.svc');
...
const recruitercntrl = {
    getrecruiters: async function (req, res) {
        try {
            let recruiters = await driverpresent.getRecruiters();
    ...
    searchrecruiter: async function (req, res) {
        try {
            ...
            let searchedcompany = await driversvc.searchRecruiter(companyname);
            if (searchedcompany) {
    ...
    loginrecruiter: async function (req, res) {
        try {
            let user = await driversvc.loginrecruiters(req.body.companyName);
            ...
            const companydeatils=await driversvc.getRecruiterName(user._id);
            if (login) {
                res.status(200).json({ status: 1, data: { companyName: req.body.companyName, phone : companydeatils.phone, token: token } });
            }
            else {
