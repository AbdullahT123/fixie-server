...
const recruitercntrl = {
    ...
    loginrecruiter: async function (req, res) {
        try {
            ...
            const companydeatils=await recruitersvc.getRecruiterName(user._id);
            if (login) {
                res.status(200).json({ status: 1, data: { companyName: req.body.companyName,companyAddress:companydeatils.industryType, token: token } });
            }
            else {
